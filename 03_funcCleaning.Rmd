...
```{r}
phage <- read.csv(here(out_dir,"final_phagelist.csv"))

amph.ref <- subset(phage, phage$Genus=="Janthinobacterium" & phage$representative=="reference_phage")

#write.csv(amph.ref, here(out_dir, "vcontact_alljanthino_metadata.csv"))
amph.ref2<- amph.ref[,c(2)]


#write.table(amph.ref2, here(out_dir, "allJanthinophage_nofna_list.txt"), sep="\t", quote= FALSE, row.names=FALSE, col.names=FALSE)
