...
hmm <- subset(hmm, hmm$Source=="Amphibian")


##subset list of all isolates which had NO phage (at all)
nophage <- subset(hmm, hmm$phage_seq=="no_phage")
nophage <- select(nophage, Sequencing.ID, phage_seq, Genus, BD.Inhibition)
nophage$ignore <- "ignoreme!"
colnames(nophage) <- c("Sequencing.ID2", "phage_seq2", "Genus2", "BD.Inhibition_v2", "ignore")

## remove no_phage hits from hmm and keep only rep phages
...
indiv.phage <- hmm.indiv %>%
  group_by(aux, aux2, COG.broad, deets, xtremedeets, phrog_category, phrog_annot, nogname, Nog.abbrev, comp, comp2, Genus, Sequencing.ID, phage_seq, BD.Inhibition, intact_est2) %>%
...
indiv.phage.freq$Genus <- ifelse(indiv.phage.freq$ignore=="ignoreme!", indiv.phage.freq$Genus2, indiv.phage.freq$Genus)
indiv.phage.freq$BD.Inhibition <- ifelse(indiv.phage.freq$ignore=="ignoreme!", indiv.phage.freq$BD.Inhibition_v2, indiv.phage.freq$BD.Inhibition)
...
indiv.iso <- hmm.indiv %>%
  group_by(aux, aux2, COG.broad, deets, xtremedeets, phrog_category, nogname, phrog_annot, Nog.abbrev, comp, comp2, Genus, Sequencing.ID, BD.Inhibition, intact_est2) %>%
  summarise(new.freq = sum(tally))
...
indiv.iso.freq$Genus <- ifelse(indiv.iso.freq$ignore=="ignoreme!", indiv.iso.freq$Genus2, indiv.iso.freq$Genus)
indiv.iso.freq$BD.Inhibition <- ifelse(indiv.iso.freq$ignore=="ignoreme!", indiv.iso.freq$BD.Inhibition_v2, indiv.iso.freq$BD.Inhibition)
...
iso.func <- unite(test_phage.func.iso_facet, label, Genus, Sequencing.ID, sep = ", ", remove=FALSE)
...
iso.func$facet_group <- factor(iso.func$facet_group,
  ...
  labels = c( 'A. Intact Genes', 'B. Intact Auxillary Genes', 'C. Degraded Genes', 'D. Degraded Auxillary Genes'))


gg.isofunc <- ggplot(data=iso.func, aes(fill=aux, x = label, y=perc.fq)) + geom_bar(stat="identity")+ coord_flip() +xlab("")+ylab("Percent of Total Prophage Genes")+theme2 + scale_fill_manual(values = auxcolors2) + guides(fill=guide_legend(ncol=4))+ theme(axis.text.y = element_text(size=16)) + facet_wrap(~ facet_group, ncol = 4, scales = "free_x") 
gg.isofunc

#ggsave(here(fig_dir, "F3.pdf"), width=16, height=17)
#ggsave(here(fig_dir, "F3.png"), width=16, height=17)
...
sum.hits <- sum(aux.summ$cat.hits.per.genera)
